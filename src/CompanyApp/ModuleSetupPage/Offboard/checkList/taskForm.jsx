...
class OffboardMSTaskForm extends Component {
    constructor(props) {
        super(props);
        this.state = {
            id: 0,
    ...
    save = (data, action) => {
        ...
        if (data.employeeId && data.employeeId.length > 0) {
            OffboardMSTaskList.employeeId = data.employeeId;
        }
        if (data.departments && data.departments.length > 0) {
            OffboardMSTaskList.departments = data.departments.map(dept => dept.id);
        }
        ...
        //     action.setSubmitting(false);
        // })
        // saveOffboardMSSubTask(subtaskList).then(res => {
        //     if (res.status == "OK") {
        //         toast.success(res.message);
        //     } else {
        //         toast.error(res.message);
    ...
    render() {
        ...
        const SubtaskCustomSelect = ({ field, setFieldValue, subtaskIndex }) => {
            ...
            let subtaskOptions = [];
            if (subtaskAssignType === '3') {
                subtaskOptions = subtaskBranches;
            } else if (subtaskAssignType === '2') {
                        ...
                        <Form autoComplete='off'>
                            ...
                            <FormGroup>
                                ...
                                <div className='d-flex mb-2'>
                                    <Field className='mr-1' type="radio" value="1" name="dueOn" onChange={(e) => setFieldValue("dueOn", e.target.value)} /> Before LWD
                                    <Field className='ml-4 mr-1' type="radio" value="2" name="dueOn" onChange={(e) => setFieldValue("dueOn", e.target.value)} /> After LWD
                                    <Field className='ml-4 mr-1' type="radio" value="3" name="dueOn" onChange={(e) => setFieldValue("dueOn", e.target.value)} /> Date of LWD
                                </div>
                                    ...
                                    <div className='subTaskOnboardBorder' key={index}>
                                        ...
                                        <FormGroup>
                                            ...
                                            <div className='d-flex'>
                                                <Field className='mr-1' type="radio" value="1" name={`subtasks[${index}].dueOn`} onChange={(e) => setFieldValue(`subtasks[${index}].dueOn`, e.target.value)} /> Before LWD
                                                <Field className='ml-4 mr-1' type="radio" value="2" name={`subtasks[${index}].dueOn`} onChange={(e) => setFieldValue(`subtasks[${index}].dueOn`, e.target.value)} /> After LWD
                                                <Field className='ml-4 mr-1' type="radio" value="3" name={`subtasks[${index}].dueOn`} onChange={(e) => setFieldValue(`subtasks[${index}].dueOn`, e.target.value)} /> Date of LWD
                                            </div>
...
export default OffboardMSTaskForm;