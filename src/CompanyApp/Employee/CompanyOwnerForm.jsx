...
import EmployeeDropDownByAdmin from '../ModuleSetup/Dropdown/EmployeeDropDownByAdmin';
import CompanyDropDownByAdmin from '../ModuleSetup/Dropdown/CompanyDropDownByAdmin';
import EmployeeDropDownByCompany from '../ModuleSetup/Dropdown/EmployeeDropDownByCompany';
import { saveOwner } from './service';
import { getIsMultiEntity } from '../../utility';


export default class CompanyOwnerForm extends Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    save = (data, action) => {
        ...
        if(this.state.isMultiEntity && !data.autoApproval && (!data.reportingHeadCompanyId ||!data.reportingHeadId )){
            return toast.error("Please select the Reporting head company and Employee for the Owner");
        }
        if(!data.employeeId){
            return toast.error("Please select the Employee to add Owner/Admin");
        }
    ...
    render() {
        let { ownerDetails ,isMultiEntity} = this.state;
        return (
            <Formik
                                ...
                                <Field required={!ownerDetails.autoApproval} name="reportingHeadCompanyId" className="form-control"
                                    render={() => {
                                        return <CompanyDropDownByAdmin
                                            nodefault={false}
                                            onChange={this.handleReportingHeadCompanyIdChange}
