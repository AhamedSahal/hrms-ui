...
import { deletePotentialScore,  getPotentialScoreList } from './service';
import { getTitle, verifyOrgLevelEditPermission, verifyOrgLevelViewPermission } from '../../../../utility';
import AccessDenied from '../../../../MainPage/Main/Dashboard/AccessDenied';
import PotentialScoreForm from './potentialForm';
const { Header, Body, Footer, Dialog } = Modal;
export default class PotentialRatingScore extends Component {
    constructor(props) {
        super(props);
    ...
    fetchList = () => {
        if (verifyOrgLevelViewPermission("Module Setup Performance")) {
            getPotentialScoreList(this.state.q, this.state.page, this.state.size, this.state.sort).then(res => {
    ...
    delete = (overallscore) => {
        confirmAlert({
            title: `Delete Potential Score ${overallscore.name}`,
            message: 'Are you sure, you want to delete this Rating Scale?',
            buttons: [
                {
                    label: 'Yes',
                    onClick: () => deletePotentialScore(overallscore.id).then(res => {
                        if (res.status == "OK") {
    ...
    render() {
        ...
        return (
            <>
                <div className="row">
                    <h3 className='mt-2 col-md-4'>Potential Rating</h3>
                ...
                <Modal enforceFocus={false} size={"md"} show={this.state.showForm} onHide={this.hideForm} >
                    ...
                    <Body>
                        <PotentialScoreForm updateList={this.updateList} overallscore={this.state.overallscore}>
                        </PotentialScoreForm>
                    </Body>
