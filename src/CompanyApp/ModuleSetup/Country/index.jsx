...
import { deleteCountry, getCountryList } from './service';
...
export default class Country extends Component {
  ...
  fetchList = () => {
    if (verifyOrgLevelViewPermission("Module Setup Manage")) {
      getCountryList(this.state.q, this.state.page, this.state.size, this.state.sort).then(res => {
        if (res.status == "OK") {
          this.setState({
            data: res.data.list,
  ...
  updateList = (country) => {
    let { data } = this.state;
    let index = data.findIndex(d => d.id == country.id);
    if (index > -1)
      data[index] = country;
    else {
      data = [country, ...data];
  ...
  delete = (country) => {
    confirmAlert({
      ...
      buttons: [
        {
          label: 'Yes',
          onClick: () => deleteCountry(country.id).then(res => {
            if (res.status == "OK") {
            ...
            {verifyOrgLevelEditPermission("Module Setup Manage") && <><a href="#" className="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
              ...
              <div className="dropdown-menu dropdown-menu-right">
                <a className="dropdown-item" href="#" onClick={() => {
                  this.setState({ country: text, showForm: true })
                }} >
    ...
    return (
      ...
      <div className="tablePage-header">
        <div className="row pageTitle-section">
          <div className="col">
            <h3 className="tablePage-title">Country</h3>
          </div>
      ...
      <div className="row">
        ...
        <Modal enforceFocus={false} size={"md"} show={this.state.showForm} onHide={this.hideForm} >
          <Header closeButton>
            <h5 className="modal-title">{this.state.country ? 'Edit' : 'Add'} Country</h5>
          </Header>
          <Body>
            <CountryForm updateList={this.updateList} country={this.state.country}>
            </CountryForm>
          </Body>
        </Modal>
        </div>
