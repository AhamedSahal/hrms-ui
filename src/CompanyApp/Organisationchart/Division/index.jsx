...
import { deleteDivision, getDivisionList } from './service';
...
export default class Division extends Component {
  ...
  fetchList = () => {
    ...
    getDivisionList(this.state.q, this.state.page, this.state.size, this.state.sort).then(res => {
  ...
  updateList = (division) => {
    let { data } = this.state;
    let index = data.findIndex(d => d.id == division.id);
    if (index > -1)
      data[index] = division;
    else {
      data = [division, ...data];
  ...
  delete = (division) => {
    confirmAlert({
      ...
      buttons: [
        {
          label: 'Yes',
          onClick: () => deleteDivision(division.id).then(res => {
            if (res.status == "OK") {
  ...
  render() {
    ...
    const menuItems = (text, record) => [
      <div ><a className="muiMenu_item" href="#" onClick={() => {
        this.setState({ division: text, showForm: true })
      }} >
    ...
    return (
      <>
        <div className="page-container content container-fluid">
          ...
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Division</h3>
              </div>
              <div className="float-right col-auto mt-2">
                ...
                {verifyOrgLevelEditPermission("Organize Org Setup") &&
                  ...
                  }}><i className="fa fa-plus" /> Add Division</a>}
        ...
        <Modal enforceFocus={false} size={"md"} show={this.state.showForm} onHide={this.hideForm} >
          ...
          <Header closeButton>
            <h5 className="modal-title">{this.state.division ? 'Edit' : 'Add'} Division</h5>

          </Header>
          <Body>
            <DivisionForm updateList={this.updateList} division={this.state.division}>
            </DivisionForm>
          </Body>
