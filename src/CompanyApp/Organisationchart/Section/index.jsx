...
import { deleteSection, getSectionList } from './service';
...
export default class Section extends Component {
  ...
  fetchList = () => {
    ...
    getSectionList(this.state.q, this.state.page, this.state.size, this.state.sort).then(res => {
  ...
  updateList = (section) => {
    let { data } = this.state;
    let index = data.findIndex(d => d.id == section.id);
    if (index > -1)
      data[index] = section;
    else {
      data = [section, ...data];
  ...
  delete = (section) => {
    confirmAlert({
      ...
      buttons: [
        {
          label: 'Yes',
          onClick: () => deleteSection(section.id).then(res => {
            if (res.status == "OK") {
  ...
  render() {
    ...
    const menuItems = (text, record) => [
      <div ><a className="muiMenu_item" href="#" onClick={() => {
        this.setState({ section: text, showForm: true })
      }} >
    ...
    return (
      <>
        <div className="page-container content container-fluid">
          ...
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Section</h3>
              </div>
              <div className="float-right col-auto mt-2">
                ...
                {verifyOrgLevelEditPermission("Organize Org Setup") &&
                  ...
                  }}><i className="fa fa-plus" /> Add Section</a>}
        ...
        <Modal enforceFocus={false} size={"md"} show={this.state.showForm} onHide={this.hideForm} >
          ...
          <Header closeButton>
            <h5 className="modal-title">{this.state.section ? 'Edit' : 'Add'} Section</h5>

          </Header>
          <Body>
            <SectionForm updateList={this.updateList} section={this.state.section}>
            </SectionForm>
          </Body>
