...
import GradesForm from './form';
import { deleteGrades, getGradesList } from './service';
...
export default class Grades extends Component {
  ...
  fetchList = () => {
    ...
    getGradesList(this.state.q, this.state.page, this.state.size, this.state.sort).then(res => {
  ...
  updateList = (grades) => {
    let { data } = this.state;
    let index = data.findIndex(d => d.id == grades.id);
    if (index > -1)
      data[index] = grades;
    else {
      data = [grades, ...data];
  ...
  delete = (grades) => {
    confirmAlert({
      title: `Delete grades ${grades.name}`,
      message: 'Are you sure, you want to delete this grades?',
      buttons: [
        {
          label: 'Yes',
          onClick: () => deleteGrades(grades.id).then(res => {
            if (res.status == "OK") {
  ...
  render() {
    ...
    const menuItems = (text, record) => [
      <div ><a className="muiMenu_item" href="#" onClick={() => {
        this.setState({ grades: text, showForm: true })
      }} >
    ...
    return (
      <>
        <div className="page-container content container-fluid">
          ...
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Grades</h3>
              </div>
              <div className="float-right col-auto mt-2">
                ...
                {verifyOrgLevelEditPermission("Organize Org Setup") &&
                  ...
                  }}><i className="fa fa-plus" /> Add Grades</a>}
        ...
        <Modal enforceFocus={false} size={"md"} show={this.state.showForm} onHide={this.hideForm} >
          ...
          <Header closeButton>
            <h5 className="modal-title">{this.state.grades ? 'Edit' : 'Add'} Grades</h5>

          </Header>
          <Body>
            <GradesForm updateList={this.updateList} grades={this.state.grades}>
            </GradesForm>
          </Body>
