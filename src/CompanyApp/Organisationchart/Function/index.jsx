...
export default class Function extends Component {
  ...
  fetchList = () => {
    ...
    getFunctionList(this.state.q, this.state.page, this.state.size, this.state.sort).then(res => {
  ...
  updateList = (functions) => {
    let { data } = this.state;
    let index = data.findIndex(d => d.id == functions.id);
    if (index > -1)
  ...
  delete = (functions) => {
    confirmAlert({
      title: `Delete function ${functions.name}`,
      message: 'Are you sure, you want to delete this function?',
      buttons: [
        {
          label: 'Yes',
          onClick: () => deleteFunction(functions.id).then(res => {
            if (res.status == "OK") {
  ...
  render() {
    ...
    const menuItems = (text, record) => [
      <div ><a className="muiMenu_item" href="#" onClick={() => {
        this.setState({ functions: text, showForm: true })
      }} >
    ...
    return (
      <>
        <div className="page-container content container-fluid">
          ...
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Function</h3>
              </div>
              <div className="float-right col-auto mt-2">
                ...
                {verifyOrgLevelEditPermission("Organize Org Setup") &&
                  ...
                  }}><i className="fa fa-plus" /> Add Function</a>}
        ...
        <Modal enforceFocus={false} size={"md"} show={this.state.showForm} onHide={this.hideForm} >
          ...
          <Header closeButton>
            <h5 className="modal-title">{this.state.functions ? 'Edit' : 'Add'} Function</h5>

          </Header>
          <Body>
            <FunctionForm updateList={this.updateList} functions={this.state.functions}>
            </FunctionForm>
          </Body>
