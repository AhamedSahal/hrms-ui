...
import { getUserType, verifyEditPermission, verifyOrgLevelViewPermission, verifyViewPermission,exportToSurveyResponse} from '../../../utility';
import SurveyForm from './form';
import { deleteSurvey, getCompletedList,getSurveyByParticipatedCompleted } from './service';
...
export default class Completed extends Component {
  ...
  fetchList = () => {
    ...
    getCompletedList(this.state.q, this.state.page, this.state.size, this.state.sort).then(res => {
      if (res.status == "OK") {
        this.setState({
          data: res.data.list,
  ...
  // get Participated completed Info
    ...
    getParticipatedInfo = (id) => {
      ...
      getSurveyByParticipatedCompleted(id).then(res => {
        if (res.status === "OK") {
          let tempselectedProperties = Object.keys(res.data[0]).filter(key => !this.state.selectedProperties.includes(key) && key != "id").sort();
        
          let sotedData = [...this.state.selectedProperties,...tempselectedProperties]
            this.setState({ surveyParticipatedData: res.data });
          
            exportToSurveyResponse(res.data, sotedData,"surveyResponseReport", "surveyResponseReport")
