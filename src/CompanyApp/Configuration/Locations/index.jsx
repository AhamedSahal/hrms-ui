...
import { deleteLocation, getLocationConfigList } from './service';
import AccessDenied from '../../../MainPage/Main/Dashboard/AccessDenied';
import { verifyOrgLevelEditPermission, verifyOrgLevelViewPermission } from '../../../utility';
import LocationConfigForm from './form';
import { itemRender } from "../../../paginationfunction";

const { Header, Body } = Modal;
export default class LocationConfig extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
    ...
    fetchList = () => {
        if (verifyOrgLevelViewPermission("Settings Configuration")) {
            getLocationConfigList(this.state.q, this.state.page, this.state.size, this.state.sort).then(res => {
                if (res.status == "OK") {
                    this.setState({
                        data: res.data.list,
    ...
    updateList = (location) => {
        this.fetchList()
        let { data } = this.state;
        let index = data.findIndex(d => d.id == location.id);
        if (index > -1)
            data[index] = location;
        else {
            data = [location, ...data];
    ...
    delete = (location) => {
        confirmAlert({
            ...
            message: location.active === 1 ? `This Location is currently mapped to a Attendance Group Configuration. Are you sure, you want to delete?`:'Are you sure, you want to delete this location?',
            buttons: [
                {
                    label: 'Yes',
                    onClick: () => deleteLocation(location.id).then(res => {
                        if (res.status == "OK") {
                        ...
                        <div className="dropdow">
                            ...
                            <div className="dropdown-menu dropdown-menu-right">
                                <a className="dropdown-item" href="#" onClick={() => {
                                    this.setState({ location: obj, showForm: true })
                                }} >
                ...
                < div className="page-container content container-fluid" >
                    ...
                    < div className="tablePage-header" >
                        <div className="row pageTitle-section">
                            <div className="col">
                                <h3 className="tablePage-title">Manage Location </h3>
                            </div>
                    ...
                    <Modal enforceFocus={false} size={"xl"} show={this.state.showForm} onHide={this.hideForm} >
                        <Header closeButton>
                            <h5 className="modal-title">{this.state.location ? 'Edit' : 'Add'} Location</h5>
                        </Header>
                        <Body>
                            <LocationConfigForm updateList={this.updateList} location={this.state.location}>
                            </LocationConfigForm>
                        </Body>
                    </Modal>
                </div>
