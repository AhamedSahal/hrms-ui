...
export default class ApplicantOffered extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        sorter: true,
        render: (text, record) => {
          return <span style={{ color: "#e56f00", background: "#fff4eb", padding: "3px" }}>Offer Released</span>
        },
      },
      {
        title: "Stage",
        sorter: true,
        render: (text, record) => {
          return <span>{text && text.irecruiter ? text.irecruiter : "-"}</span>
        },
      },
      {
        title: "Offered Date",
        sorter: true,
        render: (text, record) => {
          return <span>{text && text.imodifiedOn ? moment(text.imodifiedOn).format("ll") : text.emodifiedOn ? moment(text.emodifiedOn).format("ll") : "-"}</span>
        },
      },
      {
        title: "Applied On",
