...
import OfferletterForm from "../../../Onboarding/Offerletter/form";
...
export default class ApplicantEvaluating extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  handleStatusUpdate = (id, message, status,offerLetterData) => {
    let dataParameters = {
      id: id,
      status: status,
      currentStatus: this.state.status,
      offerLetterCanId:  offerLetterData?offerLetterData.candidateId:""
    }
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        sorter: true,
        render: (text, record) => {
          return <span>{(text.einterviewCompleted == true || text.iinterviewCompleted == true)?<span style={{ color: "#26af48", background: "rgba(15, 183, 107, 0.12)", padding: "3px" }}>Completed</span> :<span style={{ color: "#e56f00", background: "#fff4eb", padding: "3px" }}>Test Sent</span>}</span>
        },
      },
            ...
            <><a href="#" className="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
              ...
              <div className="dropdown-menu dropdown-menu-right">
               ...
               {isCompanyAdmin && <a className="dropdown-item"
                ...
                >
                  <BsFillEnvelopePaperFill /> Create offer
                </a>}
                {/* form */}
             ...
             {isCompanyAdmin &&  <a className="dropdown-item" href="#"
               ...
               {isCompanyAdmin && <a className="dropdown-item" href="#"
                ...
                {/* On schedule */}
               {(text.einterviewCompleted == false || text.iinterviewCompleted == false) && <a className="dropdown-item" href="#"
                  onClick={() => {
                    this.setState({ applicantName: text && text.efirstName ? text.efirstName : text.ifirstName ? text.ifirstName : "-" })
    ...
    return (
      <div className="page-container content container-fluid">
        ...
        <Modal enforceFocus={false} size={"lg"} show={this.state.showScheduleForm} onHide={this.hideScheduleForm} >
          ...
          <Body>
            <HApplicantScheduleForm hideScheduleForm={this.hideScheduleForm} applicantId={this.state.applicantId} happlicantMainId={this.state.happlicantMainId} isInternal={this.state.isInternal} applicantName={this.state.applicantName} hiringManagerId = {this.state.hiringManagerId}></HApplicantScheduleForm>

          </Body>
        </Modal>

        {/* offer letter form */}
        <Modal enforceFocus={false} size={"md"} show={this.state.showOfferLetterForm} onHide={this.hideForm} >


          <Header closeButton>
            <h5 className="modal-title">Create Offer Letter</h5>

          </Header>
          <Body>
            <OfferletterForm updateList={this.updateList} Offerletter={this.state.Offerletter} applicantId={this.state.applicantId} isInternal={this.state.isInternal} handleStatusUpdate={this.handleStatusUpdate}>
            </OfferletterForm>
          </Body>
        </Modal>
