...
export default class JobClose extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  fetchList = () => {
    ...
    getJobsList(this.state.q, this.state.page, this.state.size, this.state.sort,2).then(res => {
      if (res.status == "OK") {
        this.setState({
          data: res.data.list,
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        sorter: true,
        render: (text, record) => {
          return <span>{text && text.employmentType ? (record.workplaceType === 1?"Full Time":record.workplaceType === 2?"Probation":record.workplaceType === 3?"Intern":record.workplaceType === 4?"Contract":record.workplaceType === 5?"Part Time":"-") : "-"}</span>  
           },
      },
      {
        title: "Department",
      ...
      },
      {
        ...
        render: (text, record) => {
          let nextDate;
          if(text.expiryDate){
            let expiryDate = new Date(text.expiryDate)
             nextDate = new Date(expiryDate)
            nextDate.setDate(expiryDate.getDate() + 1)
          }
          return <span>{text && text.expiryDate ?moment(nextDate.toDateString()).format("ll"):"-" }</span>  
      ...
      },
      {
        ...
        sorter: true,
        render: (text, record) => {
          return <span>Expiry date exceeded</span>  
           }
      },
      {
        title: 'Action',
