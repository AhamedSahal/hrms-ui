...
export default class OnHold extends Component {
  constructor() {
    super();
    this.state = {
      isVisible: false,
  ...
  componentDidMount(){
   ...
   fetchList = () => {
    if(verifyOrgLevelViewPermission("Workflow")){
      (this.state.workFlowId != 0 && getWorkFlowList(this.state.q, this.state.page, this.state.size, this.state.sort,this.state.status,0).then(res => {
        if (res.status == "OK") { 
            this.setState({
                data: res.data,
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        sorter: true,
        render: (text,action) => {
          return <span className='badge bg-inverse-warning'><i className="pr-2 fa fa-hourglass-o text-warning"></i> OnHold</span>;
        }
        ...
        <div className="page-container content container-fluid">
          ...
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">On Hold</h3>
              </div>
