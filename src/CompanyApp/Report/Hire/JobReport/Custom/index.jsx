...
import { camelize, exportToCsv, getTitle,toLocalTime,verifyViewPermission,setAllChecked, getCompanyId, getMultiEntityCompanies } from '../../../../../utility';
import BranchDropdown from '../../../../ModuleSetup/Dropdown/BranchDropdown';
import DepartmentDropdown from '../../../../ModuleSetup/Dropdown/DepartmentDropdown';
import PdfDocument from '../../../pdfDocument';
import { getHireReport } from '../../service';
...
export default class HireReport extends Component {
  constructor(props) {
  super(props); 
    var today = new Date();
    ...
    this.state = {
      ...
      selectedPropertiestemp: ["jobTitle","department","jobCode","jobType","openingDate","expiryDate","noOfOpenings","employmentType","hiringManager","email","workplaceType","qualification","goodSkills","mustSkills","evaluationParameter","evaluationScaleParameter1","evaluationScaleParameter2","evaluationScaleParameter3","evaluationScaleParameter4","evaluationScaleParameter5","evaluationScaleParameter6","evaluationScaleParameter7","evaluationScaleParameter8","evaluationScaleParameter9","evaluationScaleParameter10"],
  ...
  fetchList = () => { 
    if(verifyViewPermission("Job Report")){
        getHireReport(this.state.branchId, this.state.departmentId, this.state.jobTitleId, this.state.q, this.state.fromDate, this.state.toDate,this.state.selectedCompanies).then(response => {
  ...
  render() {
    const { data, selectedProperties, showPdf,isFilter, showCsv,sortedProperties } = this.state;
    let selectedData = [];
          ...
          if (selectedProperties.includes(key)) {
            if (key != "date") {
              if(key == "workplaceType"){
                ...
                temp[key] = element[key] ? element[key]  == 1 ? "Full Time" : element[key]  == 2 ? "Probation" : element[key]  == 3  ? "Intern" : element[key]  == 4 ? "Contract" : element[key]  == 5 ? "Part Time" : "-" : "-"
              }else{
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Job Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Job Report</li>
                </ul>
              </div>
            </div>
          </div>
                ...
                {selectedData && selectedData.length > 0 &&
                  <ButtonGroup>
                    ...
                    <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                      exportToCsv(selectedData,"Attendance","Attendance")
                    }}>
