...
import PdfDocument from '../../../pdfDocument';
import { getJobStatusReport } from '../service';
...
export default class JobStatusReport extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  fetchList = () => {
    if (verifyViewPermission("Job Report")) {
        getJobStatusReport(this.state.q, this.state.fromDate, this.state.toDate > this.state.currentDate ? this.state.currentDate : this.state.toDate, this.state.jobStatus,this.state.selectedCompanies).then(response => {
  ...
  setAllChecked = (checkAll) => {
    ...
    let sortedData = []
    selectedProperties = ["hiringManager","email","jobTitle","employmenType","workplaceType","department","jobCode","jobType","openingDate","expiryDate","qualification","goodSkills","mustSkills","noOfOpenings","evaluationParameter","evaluationScaleParameter1","evaluationScaleParameter2","evaluationScaleParameter3","evaluationScaleParameter4","evaluationScaleParameter5","evaluationScaleParameter6","evaluationScaleParameter7","evaluationScaleParameter8","evaluationScaleParameter9","evaluationScaleParameter10"];
  ...
  render() {
    const { data, selectedProperties,isFilter, showPdf, showCsv } = this.state;
          ...
          if (selectedProperties.includes(key)) {
            if (key != "date") {
              if(key == "workplaceType"){
                temp[key] = element[key]? element[key] == 1? "Remote": element[key] == 2? "Onsite": element[key] == 3? "Hybrid": "-": "-"
              }else if(key == "employmenType"){
                temp[key] = element[key] ? element[key]  == 1 ? "Full Time" : element[key]  == 2 ? "Probation" : element[key]  == 3  ? "Intern" : element[key]  == 4 ? "Contrac" : element[key]  == 5 ? "Part Time" : "-" : "-"
              }else{
    ...
    return (
      ...
      <div className="insidePageDiv">
        <Helmet>
          <title>JobStatus | {getTitle()}</title>
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Job Status Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Job Status Report</li>
                </ul>
              </div>
            </div>
          </div>
          {verifyViewPermission("Job Report") && <><div className="card p-2">
            <div className="row">
              ...
              <div className="col-sm-6 col-md-4">
                <div className="form-group form-focus">
                  <select
                    ...
                    defaultValue={this.state.jobStatus}
                    onChange={(e) => {
                  ...
                  {selectedData && selectedData.length > 0 &&
                    <ButtonGroup>
                      <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                        exportToSortedCsvOrder(selectedData, "jobStatus", "jobStatus",selectedProperties)
                      }}>
          ...
          {!verifyViewPermission("Job Report") && <AccessDenied></AccessDenied>}
        </div>
        {/* /Page Content */}
      </div>
    );
