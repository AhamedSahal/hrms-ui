...
import { getOfferLetterReport } from '../service';
import CompanyMultiSelectDropDown from '../../../ModuleSetup/Dropdown/CompanyMultiSelectDropDown';
const { Header, Body, Footer, Dialog } = Modal;


export default class OfferLetterReport extends Component {
  constructor(props) {
  super(props); 
    var today = new Date();
    ...
    this.state = {
      ...
      sortedProperties:[],
      selectedPropertiestemp:["candidateId","offerletterId", "candidateName", "candidateEmailId","offerLetterDate","joiningDate","candidatePosition","salary","businessLanguages","basicSalary","allowances","noticePeriod","annualLeave","probationDays","weekWorkHours","weekOffDays","workPlace","workType","workingDays","signatureHolderId"],
  ...
  fetchList = () => {
    if(verifyViewPermission("Offer Letter Report")){
        getOfferLetterReport(this.state.q, this.state.fromDate, this.state.toDate,this.state.selectedCompanies).then(response => {
  ...
  render() {
      ...
      data.forEach((element) => {
        ...
        Object.keys(element).forEach((key) => {
          if (selectedProperties.includes(key)) {
            if(key != "joiningDate" && key != "offerLetterDate"){
              temp[key] = element[key] == null || element[key] == ""?"-":element[key];
            }else{
    ...
    return (
      <div className="reportInsidePageDiv">
        <Helmet>
          <title>OfferLetter | {getTitle()}</title>
        ...
        <div className="page-containerDocList content container-fluid" style={{marginTop: "10px"}}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Offer Letter Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Offer Letter Report</li>
                </ul>
              </div>
            </div>
          </div>
          {verifyViewPermission("Offer Letter Report") && <><div className="card p-2">
          <div className="row">
          ...
          { !verifyViewPermission("Offer Letter Report") && <AccessDenied></AccessDenied>}
        </div>
        {/* /Page Content */}
      </div>
    );
