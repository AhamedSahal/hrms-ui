...
export default class CandidateInfoReport extends Component {
  constructor(props) {
  super(props); 
    var today = new Date();
    ...
    this.state = {
      ...
      sortedProperties:[],
      selectedPropertiestemp:["candidateId","offerLetterId","firstName","middleName", "lastName","personalEmailId","fatherName","phone","candidatePosition","generateEmployeeId","candidateEmailId","comments","docmentVerify","gender","password","fileName","status"],
  ...
  fetchList = () => {
    if(verifyViewPermission("Candidate Info Report")){
        getCandidateInfoReport(this.state.q, this.state.fromDate, this.state.toDate,this.state.selectedCompanies).then(response => {
        ...
        <div className="page-containerDocList content container-fluid" style={{marginTop: "10px"}}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Candidate Info Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Candidate Info Report</li>
                </ul>
              </div>
            </div>
          </div>
          {verifyViewPermission("Candidate Info Report") && <><div className="card p-2">
          <div className="row">
          ...
          { !verifyViewPermission("Candidate Info Report") && <AccessDenied></AccessDenied>}
        </div>
        {/* /Page Content */}
      </div>
    );
