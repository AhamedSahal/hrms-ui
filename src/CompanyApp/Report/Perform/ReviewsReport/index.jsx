...
export default class ReviewsReport extends Component {
  ...
  super(props); 
    ...
    this.state = {
      ...
      selectedPropertiestemp: ["employeeId","fullName","reportingManager","department","reviewPeriodFrom","reviewPeriodTo"],
  ...
  fetchList = () => { 
    if(verifyViewPermission("Reviews Report")){
        getReviewsReport(this.state.branchId, this.state.departmentId, this.state.jobTitleId, this.state.q, this.state.fromDate, this.state.toDate,this.state.selectedCompanies).then(response => {
  ...
  handleChange = (selectedOptions) => {
    ...
    this.setState((prevState) => ({
      ...
      projectId: selectedCompanies.length > 1 ? '' : prevState.projectId,
  }));
  };
      ...
      data.forEach((element) => {
        ...
        Object.keys(element).forEach((key) => {
          if (selectedProperties.includes(key)) {
            if (key != "reviewPeriodTo" && key != "reviewPeriodFrom") {
              temp[key] = element[key] == null ? "-" : element[key];
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Performance Reviews Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Performance Reviews Report</li>
                </ul>
              </div>
            </div>
          </div>
          {verifyViewPermission("Reviews Report") && <><div className="card p-2">
          <div className="row">
          ...
          { !verifyViewPermission("Reviews Report") && <AccessDenied></AccessDenied>}
        </div>
        {/* /Page Content */}
      </div>
    );
