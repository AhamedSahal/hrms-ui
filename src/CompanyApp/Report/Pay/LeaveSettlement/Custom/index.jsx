...
import { getLeaveSettlementReport } from '../service';
...
export default class LeaveSettlementReport extends Component {
  constructor(props) {
  super(props); 
    var today = new Date();
    ...
    this.state = {
      ...
      sortedProperties:[],
      selectedPropertiestemp:["employeeId", "fullName", "email","reportingManager","department","jobTitle","leaveName","leaveStartEndMonthName","payrollMonth","noOfDays","currentMonthNetSalary","totalLeaveSalary","totalAmount","status"],
  ...
  fetchList = () => {
    if(verifyViewPermission("Leave Settlement Report")){
        getLeaveSettlementReport(this.state.branchId, this.state.departmentId, this.state.jobTitleId, this.state.q, this.state.fromDate, this.state.toDate,this.state.selectedCompanies).then(response => {
      ...
      data.forEach((element) => {
        ...
        Object.keys(element).forEach((key) => {
          if (selectedProperties.includes(key)) {
            if(key != "leaveEndDate" && key != "leaveStartDate" && key != "doj"){
        ...
        <div className="page-containerDocList content container-fluid" style={{marginTop: "20px"}}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Leave Settlement Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Leave Settlement Report</li>
                </ul>
              </div>
            </div>
          </div>
                ...
                {selectedData && selectedData.length > 0 &&
                  <ButtonGroup>
                    ...
                    <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                      exportToSortedCsvOrder(selectedData,"LeaveSettlement","LeaveSettlement",selectedProperties)
                    }}>
