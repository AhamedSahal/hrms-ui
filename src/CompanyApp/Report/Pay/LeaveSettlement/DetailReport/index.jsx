...
import PdfDocument from '../../../pdfDocument';
import { getLeaveSettlementDetailReport } from '../service';
...
export default class LeaveSettlementDetailReport extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  fetchList = () => {
    if (verifyViewPermission("Leave Settlement Report")) {
        getLeaveSettlementDetailReport(this.state.q, this.state.fromDate, this.state.toDate > this.state.currentDate ? this.state.currentDate : this.state.toDate,this.state.selectedCompanies).then(response => {
  ...
  setAllChecked = (checkAll) => {
    ...
    let sortedData = []
    selectedProperties = ["employeeId", "fullName", "email","reportingManager","department","jobTitle","payrollMonth","leaveName","approvedLeaveDate","approvedDate","noOfDays","currentMonthNetSalary","leaveSalaryAdvance","totalLeaveSettlementAmount"];
  ...
  render() {
    ...
    if (data && selectedProperties) {
          ...
          if (selectedProperties.includes(key)) {
            if (key != "approvedDate") {
              temp[key] = element[key] == null || element[key] == "undefined" ? "-" : element[key];
               if(key == "totalLeaveSettlementAmount"){
      ...
      if(selectedData.length > 0){
        let tempTotal = {
          leaveSalaryAdvance: "Total : ",
          totalLeaveSettlementAmount: leaveSettlementTotalHours
        }
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Leave Settlement Detail Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Leave Settlement Detail Report</li>
                </ul>
              </div>
            </div>
          </div>
                  ...
                  {selectedData && selectedData.length > 0 &&
                    <ButtonGroup>
                      <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                        exportToSortedCsvOrder(selectedData, "leaveSettlementDetail", "leaveSettlementDetail",selectedProperties)
                      }}>
