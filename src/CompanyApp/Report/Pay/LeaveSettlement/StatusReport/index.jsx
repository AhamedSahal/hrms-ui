...
import PdfDocument from '../../../pdfDocument';
import { getLeaveSettlementStatusReport } from '../service';
...
export default class LeaveSettlementStatusReport extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  fetchList = () => {
    if (verifyViewPermission("Leave Settlement Report")) {
        getLeaveSettlementStatusReport(this.state.q, this.state.fromDate, this.state.toDate > this.state.currentDate ? this.state.currentDate : this.state.toDate, this.state.leaveSettlementStatus,this.state.selectedCompanies).then(response => {
  ...
  setAllChecked = (checkAll) => {
    ...
    let sortedData = []
    selectedProperties = ["employeeId", "fullName", "email","reportingManager","department","jobTitle","leaveName","leaveStartEndMonthName","payrollMonth","noOfDays","currentMonthNetSalary","totalLeaveSalary","totalAmount","status"];
  ...
  render() {
    ...
    let selectedData = [];
    let leaveSettlementTotalHours = 0;
    if (data && selectedProperties) {
        ...
        Object.keys(element).forEach((key) => {
          if (selectedProperties.includes(key)) {
            if (key != "date") {
              ...
              if(key == "totalAmount"){
                leaveSettlementTotalHours = leaveSettlementTotalHours + Number(element[key]);
              }
            } else {
      ...
      if(selectedData.length > 0){
        let tempTotal = {
          totalLeaveSalary: "Total Hours: ",
          totalAmount: leaveSettlementTotalHours
        }
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Leave Settlement Status Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Leave Settlement Status Report</li>
                </ul>
              </div>
            </div>
          </div>
          {verifyViewPermission("Leave Settlement Report") && <><div className="card p-2">
            <div className="row">
              ...
              <div className="col-sm-6 col-md-4">
                <div className="form-group form-focus">
                  <select
                    ...
                    defaultValue={this.state.leaveSettlementStatus}
                    onChange={(e) => {
                      this.setState({ leaveSettlementStatus: e.target.value })
                    }}
                  >
                  ...
                  {selectedData && selectedData.length > 0 &&
                    <ButtonGroup>
                      <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                        exportToSortedCsvOrder(selectedData, "leaveSettlementStatus", "leaveSettlementStatus",selectedProperties)
                      }}>
          ...
          {!verifyViewPermission("Leave Settlement Report") && <AccessDenied></AccessDenied>}
        </div>
        {/* /Page Content */}
      </div>
    );
