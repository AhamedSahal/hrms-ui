...
import { getManageWorkDaysReport } from '../service';
...
export default class ManageWorkDaysReport extends Component {
  constructor(props) {
  super(props); 
    var today = new Date();
    ...
    this.state = {
      ...
      sortedProperties:[],
      selectedPropertiestemp:["employeeId", "fullName", "email","salaryMonth","daysWorked"],
  ...
  fetchList = () => {
    if(verifyViewPermission("Manage Work Days Report")){
        getManageWorkDaysReport(this.state.branchId, this.state.departmentId, this.state.jobTitleId, this.state.q, this.state.fromDate, this.state.toDate,this.state.selectedCompanies).then(response => {
  ...
  render() {
    ...
    return (
      <div className="reportInsidePageDiv">
        <Helmet>
          <title>Manage Work Days Report | {getTitle()}</title>
          <meta name="description" content="Login page" />
        </Helmet>
        {/* Page Content */}
        <div className="page-containerDocList content container-fluid" style={{marginTop: "10px"}}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Manage Work Days Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Manage Work Days Report</li>
                </ul>
              </div>
            </div>
          </div>
          {verifyViewPermission("Manage Work Days Report") && <><div className="card p-2">
          <div className="row">
                ...
                {selectedData && selectedData.length > 0 &&
                  <ButtonGroup>
                    ...
                    <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                      exportToSortedCsvOrder(selectedData,"ManageWorkDays","ManageWorkDays",selectedProperties)
                    }}>
          ...
          { !verifyViewPermission("Manage Work Days Report") && <AccessDenied></AccessDenied>}
        </div>
        {/* /Page Content */}
      </div>
    );
