...
import { exportToSortedCsvOrder, getCompanyId, getMultiEntityCompanies, getTitle, verifyViewPermission } from '../../../../../utility';
// import PdfDocument from '../../../pdfDocument';
import PdfDocument from '../../../pdfDocument';
import { getFinalSettlementStatusReport } from '../service';
...
export default class FinalSettlementStatusReport extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  fetchList = () => {
    if (verifyViewPermission("Final Settlement Report")) {
        getFinalSettlementStatusReport(this.state.q, this.state.fromDate, this.state.toDate > this.state.currentDate ? this.state.currentDate : this.state.toDate, this.state.finalSettlementStatus,this.state.selectedCompanies).then(response => {
  ...
  setAllChecked = (checkAll) => {
    ...
    let sortedData = []
    selectedProperties = ["employeeId", "fullName", "email","reportingManager","department","jobTitle","dateOfJoining","resignationDate","lastWorkingDate","status","annualLeaveEncashment","gratuity","pendingSalary","currentMonthSalary","noticePay","otherPayments","otherDeductions","totalFinalSettlement"];
    ...
    if (data && selectedProperties) {
          ...
          if (selectedProperties.includes(key)) {
            if (key != "dateOfJoining" && key != "resignationDate" && key != "lastWorkingDate") {
              ...
              if(key == "totalFinalSettlement"){
                finalSettlementTotalHours = finalSettlementTotalHours + Number(element[key]);
              }
            } else {
      ...
      if(selectedData.length > 0){
        let tempTotal = {
          ...
          totalFinalSettlement: finalSettlementTotalHours
        }
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Final Settlement Status Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Final Settlement Status Report</li>
                </ul>
              </div>
            </div>
          </div>
          {verifyViewPermission("Final Settlement Report") && <><div className="card p-2">
            <div className="row">
              ...
              <div className="col-sm-6 col-md-4">
                <div className="form-group form-focus">
                  <select
                    ...
                    defaultValue={this.state.finalSettlementStatus}
                    onChange={(e) => {
                      this.setState({ finalSettlementStatus: e.target.value })
                    }}
                  >
                  ...
                  {selectedData && selectedData.length > 0 &&
                    <ButtonGroup>
                      <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                        exportToSortedCsvOrder(selectedData, "FinalSettlementStatus", "FinalSettlementStatus",selectedProperties)
                      }}>
          ...
          {!verifyViewPermission("Final Settlement Report") && <AccessDenied></AccessDenied>}
        </div>
        {/* /Page Content */}
      </div>
    );
