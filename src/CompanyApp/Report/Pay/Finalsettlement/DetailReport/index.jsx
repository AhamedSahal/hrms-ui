...
import PdfDocument from '../../../pdfDocument';
import { getFinalSettlementDetailReport } from '../service';
...
export default class FinalSettlementDetailReport extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  fetchList = () => {
    if (verifyViewPermission("Final Settlement Report")) {
        getFinalSettlementDetailReport(this.state.q, this.state.fromDate, this.state.toDate > this.state.currentDate ? this.state.currentDate : this.state.toDate,this.state.selectedCompanies).then(response => {
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Final Settlement Detail Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Final Settlement Detail Report</li>
                </ul>
              </div>
            </div>
          </div>
                  ...
                  {selectedData && selectedData.length > 0 &&
                    <ButtonGroup>
                      <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                        exportToSortedCsvOrder(selectedData, "FinalSettlementDetail", "FinalSettlementDetail",selectedProperties)
                      }}>
