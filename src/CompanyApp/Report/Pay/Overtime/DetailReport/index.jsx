...
import { getOvertimeDetailReport } from '../service';
...
export default class OvertimeDetailReport extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  fetchList = () => {
    if (verifyViewPermission("Overtime Report")) {
        getOvertimeDetailReport(this.state.q, this.state.fromDate, this.state.toDate > this.state.currentDate?this.state.currentDate:this.state.toDate,this.state.formattedOffset,this.state.selectedCompanies).then(response => {
  ...
  setAllChecked = (checkAll) => {
    ...
    if (data && data.length > 0 && checkAll) {
      //  let tempselectedProperties = Object.keys(data[0]).map((key) => {
    ...
    if (data && selectedProperties) {
      data.forEach((element) => {
        ...
        Object.keys(element).forEach((key) => {
          if (selectedProperties.includes(key)) {
            temp[key] = element[key] == null || element[key] == "undefined" ? "-" : element[key] == 0?"00:00": (element[key]) > 0? Number(element[key]).toFixed(2):element[key];
            if(element[key] > 0){
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Overtime Detail Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Overtime Detail Report</li>
                </ul>
              </div>
            </div>
          </div>
                  ...
                  {selectedData && selectedData.length > 0 &&
                    <ButtonGroup>
                      <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                        exportToSortedCsv(selectedData, "Overtime", "Overtime")
                      }}>
