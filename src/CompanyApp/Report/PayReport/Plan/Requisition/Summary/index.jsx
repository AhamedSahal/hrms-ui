...
import PdfDocument from '../../../../pdfDocument';
import { getRequisitionSummaryReport } from '../service';
...
export default class RequisitionSummaryReport extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  fetchList = () => {
    if (verifyViewPermission("WorkForce Plan Report")) {
        getRequisitionSummaryReport(this.state.q, this.state.fromDate, this.state.toDate > this.state.currentDate?this.state.currentDate:this.state.toDate,this.state.departmentId,this.state.selectedCompanies).then(response => {
  ...
  setAllChecked = (checkAll) => {
    ...
    let sortedData = [];
    let selectedProperties = ["department","budgeted","nonBudgeted","budgetedNoOfResource","nonBudgetedNoOfResource","processInitiated","processStarted","processStopped"];
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Requisition Summary Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Requisition Summary Report</li>
                </ul>
              </div>
            </div>
          </div>
                  ...
                  {selectedData && selectedData.length > 0 &&
                    <ButtonGroup>
                      <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                        exportToSortedCsv(selectedData, "RequisitionSummary", "RequisitionSummary")
                      }}>
