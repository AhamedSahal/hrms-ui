...
import PdfDocument from '../../../../pdfDocument';
import { getBudgetStatusReport } from '../service';
...
export default class ProjectBudgetStatusReport extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  fetchList = () => {
    if (verifyViewPermission("Project Report")) {
        getBudgetStatusReport(this.state.q, this.state.fromDate, this.state.toDate > this.state.currentDate ? this.state.currentDate : this.state.toDate, this.state.projectBudgetStatus,this.state.selectedCompanies).then(response => {
  ...
  setAllChecked = (checkAll) => {
    ...
    let sortedData = []
    selectedProperties = ["project","activityCount","projectBudget","activityCost","projectStatus"];
  ...
  handleChange = (selectedOptions) => {
    ...
    this.setState((prevState) => ({
      ...
      projectBudgetStatus: selectedCompanies.length > 1 ? '' : prevState.projectBudgetStatus,
  ...
  render() {
        ...
        Object.keys(element).forEach((key) => {
          if (selectedProperties.includes(key)) {
            if (key == "projectStatus") {
              temp[key] = element[key] == "INPROGRESS" ? "Inprogress" : element[key] == "NOT_STARTED"?"Not Started":element[key] == "COMPLETED"?"Completed":"-" 
            }else if(key == "projectCostStatus" ){
              temp[key] = element[key] == "UNDER_ESTIMATION" ? "Under Expense" : element[key] == "ATPAR"?"At Par":element[key] == "OVER_ESTIMATION"?"Over Expense":"-"
            }
             else {
    ...
    return (
      ...
      <div className="insidePageDiv">
        <Helmet>
          <title>Budget Detail | {getTitle()}</title>
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Budget Detail Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Budget Detail Report</li>
                </ul>
              </div>
            </div>
          </div>
          {verifyViewPermission("Project Report") && <><div className="card p-2">
            <div className="row">
              ...
              <div className="col-sm-6 col-md-3">
                <div className="form-group form-focus">
                  <ProjectDropdown  readOnly ={!this.state.isFilter} defaultValue={this.state.projectBudgetStatus}  companyId={this.state.companyId} onChange={e => {
                    this.setState({
                      projectBudgetStatus: e.target.value
                    })
                  }}></ProjectDropdown>
                </div>
              </div>
                  ...
                  {selectedData && selectedData.length > 0 &&
                    <ButtonGroup>
                      <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                        exportToSortedCsvOrder(selectedData, "BudgetStatus", "BudgetStatus",selectedProperties)
                      }}>
          ...
          {!verifyViewPermission("Project Report") && <AccessDenied></AccessDenied>}
        </div>
        {/* /Page Content */}
      </div>
    );
