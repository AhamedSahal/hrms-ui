...
import { getTimesheetDetailReport } from '../service';
...
export default class TimesheetDataReport extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  componentDidMount() {
    ...
    this.setState({formattedOffset: timeZone});
  }
  fetchList = () => {
    if (verifyViewPermission("Timesheet Report")) {
        getTimesheetDetailReport(this.state.q, this.state.fromDate, this.state.toDate > this.state.currentDate?this.state.currentDate:this.state.toDate,this.state.formattedOffset,this.state.selectedCompanies).then(response => {
      ...
      data.forEach((element) => {
        ...
        Object.keys(element).forEach((key) => {
          if (selectedProperties.includes(key)) {
            temp[key] = element[key] == null || element[key] == "undefined" ? "-" : element[key] == 0?"WeekOff": element[key];
          }
        });
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Timesheet Detail Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Timesheet Detail Report</li>
                </ul>
              </div>
            </div>
          </div>
                  ...
                  {selectedData && selectedData.length > 0 &&
                    <ButtonGroup>
                      <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                        exportToSortedCsv(selectedData, "Timesheet", "Timesheet")
                      }}>
