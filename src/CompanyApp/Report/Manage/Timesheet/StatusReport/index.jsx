...
import PdfDocument from '../../../pdfDocument';
import { getTimesheetStatusReport } from '../service';
...
export default class TimesheetStatusReport extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  fetchList = () => {
    if (verifyViewPermission("Timesheet Report")) {
        getTimesheetStatusReport(this.state.q, this.state.fromDate, this.state.toDate > this.state.currentDate?this.state.currentDate:this.state.toDate,this.state.timesheetStatus,this.state.projectId,this.state.selectedCompanies).then(response => {
  ...
  setAllChecked = (checkAll) => {
    ...
    let sortedData= []
     selectedProperties = ["employeeId", "fullName", "email","reportingManager","jobTitle","department","date","project","activity","actualHours","approvedHours","approvalStatus"];
  ...
  render() {
    const { data, selectedProperties, showPdf, showCsv,companies } = this.state;
          ...
          if (selectedProperties.includes(key)) {
            if(key != "Date"){
              if(key == "approvalStatus"){
                temp[key] = element[key] == null || element[key] == "undefined" ? "-":element[key] == "PENDING"?"Pending":element[key] == "APPROVED"?"Approved":element[key] == "REJECTED"?"Rejected":"-"
              }else{
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Timesheet Status Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Timesheet Status Report</li>
                </ul>
              </div>
            </div>
          </div>
          {verifyViewPermission("Timesheet Report") && <><div className="card p-2">
            <div className="row">
            ...
            <div className="col-sm-6 col-md-4">
              <div className="form-group form-focus">
                  <select
                    ...
                    defaultValue={this.state.timesheetStatus}
                    onChange={(e) => {
                      this.setState({timesheetStatus: e.target.value})
                    }}
                  >
            ...
            <div className="col-sm-6 col-md-4">
              <div className="form-group form-focus">
                <ProjectDropdown  readOnly ={!this.state.isFilter} defaultValue={this.state.projectId} companyId={this.state.companyId}  onChange={e => {
                  this.setState({
                  ...
                  {selectedData && selectedData.length > 0 &&
                    <ButtonGroup>
                      <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                        exportToSortedCsvOrder(selectedData, "TimesheetStatus", "TimesheetStatus",selectedProperties)
                      }}>
          ...
          {!verifyViewPermission("Timesheet Report") && <AccessDenied></AccessDenied>}
        </div>
        {/* /Page Content */}
      </div>
    );
