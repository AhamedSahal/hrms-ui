...
import { getLeaveReport } from '../service';
...
export default class LeaveReport extends Component {
  ...
  super(props); 
    ...
    this.state = {
      ...
      jobTitleId: "",
      selectedProperties: [],
      selectedPropertiestemp : ["employeeId", "email", "fullName","startDate","endDate","leaveType","totalLeaveDays","leaveReason","appliedOn","approver","leaveStatus"],
  ...
  fetchList = () => {
    ...
    getLeaveReport(this.state.branchId, this.state.departmentId, this.state.jobTitleId, this.state.q, this.state.fromDate, this.state.toDate,this.state.selectedCompanies).then(response => {
  ...
  render() {
    const { data, selectedProperties, showPdf,isFilter, showCsv,sortedProperties,companyId,companies } = this.state;
    let selectedData = [];
        ...
        <div className="page-containerDocList content container-fluid" style={{ marginTop: "20px" }}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Leave Custom Report</h3>
                <ul hidden className="breadcrumb">
                  <li className="breadcrumb-item"><a href="/app/main/dashboard">Dashboard</a></li>
                  <li className="breadcrumb-item">Report</li>
                  <li className="breadcrumb-item active">Leave Custom Report</li>
                </ul>
              </div>
            </div>
          </div>
          {verifyViewPermission("Leave Report") && <><div className="card p-4">
          <div className="row">
                ...
                {selectedData && selectedData.length > 0 &&
                  <ButtonGroup>
                    ...
                    <Button className='add-btn' variant='warning' size='sm' onClick={() => {
                      exportToCsvSorted(selectedData, selectedProperties, "LeaveCustom","LeaveCustom")
                    }}>
