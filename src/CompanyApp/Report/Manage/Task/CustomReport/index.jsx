...
import { getTaskReport } from '../service';
...
export default class TaskReport extends Component {
  ...
  super(props); 
    ...
    this.state = {
      ...
      sortedProperties:[],
      selectedPropertiestemp:["employeeId", "fullName","email","taskName","startDate","endDate","fileName","description", "url","completedDate","status"],
  ...
  fetchList = () => {
    if(verifyViewPermission("Task Report")){
        getTaskReport(this.state.branchId, this.state.departmentId, this.state.jobTitleId, this.state.q, this.state.fromDate, this.state.toDate,this.state.selectedCompanies).then(response => {
  ...
  render() {
    const { data, selectedProperties, showPdf, showCsv,sortedProperties,companyId,companies } = this.state;
    let selectedData = [];
      ...
      data.forEach((element) => {
        ...
        Object.keys(element).forEach((key) => {
          if (selectedProperties.includes(key)) {
            if(key != "startDate" && key != "endDate" && key != "completedDate"){
              if(key == "status"){
                temp[key] = element[key] == null || element[key] == "undefined" || element[key] == ""?"-":element[key] == "PENDING"?"PENDING":element[key] == "APPROVED"?"COMPLETED":"OVERDUE"
              }else{
        ...
        <div className="page-containerDocList content container-fluid" style={{marginTop: "20px"}}>
          <div className="tablePage-header">
            <div className="row pageTitle-section">
              <div className="col">
                <h3 className="tablePage-title">Task Report</h3>
          ...
          {verifyViewPermission("Task Report") && <><div className="card p-2">
          <div className="row">
          ...
          { !verifyViewPermission("Task Report") && <AccessDenied></AccessDenied>}
        </div>
        {/* /Page Content */}
      </div>
    );
